// ./client/src/Layout/Header.js
import React from "react";
import { Link } from "react-router-dom";

export default function Header() {
  return (

	<header className="bg-gradient-to-r from-gray-700 to-gray-800">
      <nav className="max-w-m mx-auto px-3 sm:px-6 lg:px-8" aria-label="Top">
        <div className="w-full border-b border-gray-500 lg:border-none py-3 grid gird-cols-6 gap-1 grid-rows-1 ">
          <div className="  col-star-3 col-span-1  ">
            <Link to="/">
              <img
                className="h-11 w-auto"
                src="data:image/png;base64,"
                alt=""
              />
            </Link>

            <h1 className="text-green-600 font-black text-2xl pl-5">
              HackerShop
            </h1>
          </div>
          
		  <div className="col-end-6 col-span-1 ">
            <Link
              to="/products"
              className="text-base font-medium text-white hover:text-indigo-50"
            >
              Products
            </Link>
          </div>
        </div>
      </nav>
    </header>

    
  );
}


/* <header className="bg-gradient-to-r from-gray-700 to-gray-800">
      <nav className="max-w-sm mx-auto px-4 sm:px-6 lg:px-8" aria-label="Top">
        <div className="w-full py-6 flex items-center justify-between border-b border-gray-500 lg:border-none">
          <div className="flex items-center">
            <Link to="/">
              <img
                className="h-10 w-auto"
                src="data:image/png;base64,"
                alt=""
              />
            </Link>

            <h1 className="text-green-600 font-black text-2xl pl-5">
              HackerShop
            </h1>
          </div>
          <div className="justify-end">
            <Link
              to="/products"
              className="text-base font-medium text-white hover:text-indigo-50"
            >
              Products
            </Link>
          </div>
        </div>
      </nav>
    </header> */